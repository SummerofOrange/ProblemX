...
## v2.2 (2026-06-05)
  ...
  - **OCS 本地题库服务全新上线**：
    - 在“题目助手”中新增 **OCS服务** 分页，可启动本地 HTTP 题库服务供 OCS 网课助手检索 ProblemX 本地题库。
    - 自动生成可直接粘贴到 OCS 网课助手题库配置中的 JSON 配置文本，使用 `GM_xmlhttpRequest` 调用本地服务。
    - 支持配置监听地址、端口、相似度阈值和 Top-K 返回数量，并持久化到 `config.json`。
    - 提供服务状态、访问地址、日志面板和题库索引刷新入口，便于排障与调试。

  - **OCS 未命中题目记录与导出**：
    - 对每次 OCS 搜题请求进行响应跟踪，当没有匹配到达到阈值且有答案的题目时自动记录到本地。
    - 未命中题目按题干、选项和题型去重，记录最佳相似度、阈值、首次/最后出现时间和累计次数。
    - 支持在 OCS 服务页一键导出未命中题目 JSON 草稿，后续补全答案后可整理为正式题库。
    - 支持清空未命中记录，便于按课程或阶段重新收集新题。

  - **界面与体验优化**：
    - OCS 服务配置区域改为网格布局，标签左对齐、输入控件右对齐，端口/阈值/返回数量输入框加宽，避免内容截断。
    - 本地服务增加健康检查接口和 CORS 响应头，便于浏览器脚本和本地调试工具访问。
